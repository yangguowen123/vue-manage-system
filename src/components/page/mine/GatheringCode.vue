    ...
    <div id="gatheringCode" v-cloak>
		<div class="page-body" style="background: #f2f2f2;" v-show="showGatheringCodeFlag">
			<ul class="nav nav-tabs nav-justified">
				<li class="nav-item common-nav-item" v-for="(dictItem, index) in gatheringChannelDictItems" :key="index" v-bind:class="{'common-nav-item-active': dictItem == selectedGatheringChannel}" v-on:click="switchTab(dictItem)"><a class="nav-link">{{dictItem.channelName}}</a></li>
			</ul>
			<div class="form-details" v-for="(gatheringCode, index) in gatheringCodes" :key="index">
				...
				<form>
					<div class="form-group">
						<label>收款通道:</label> <select class="form-control" v-model="selectedGatheringChannel">
							...
							<option v-for="(dictItem, index) in gatheringChannelDictItems" :key="index" :value="dictItem">{{dictItem.channelName}}</option>
						</select>
					</div>
					...
					<div class="form-group" v-show="selectedGatheringChannel.channelCode == 'bankCard' || selectedGatheringChannel.channelCode == 'alipayCard'">
						<label>收款人:</label> <input type="text" class="form-control" v-model="editGatheringCode.payee" placeholder="此处必须是收款码的真实姓名">
					</div>
					<div class="form-group" v-show="selectedGatheringChannel.channelCode == 'bankCard' || selectedGatheringChannel.channelCode == 'alipayCard'">
...
<script>
import gatheringCode from '@/components/common/js/gathering-code.js';
export default gatheringCode;
</script>
<style scoped>
