<template>
    <div id="personal-info" v-cloak>
		<div class="page-body">
			<div class="personal-info-form" v-show="showPersonalInfoFlag">
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label>用户名</label> <span>{{accountInfo.userName}}</span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label>真实姓名</label> <span>{{accountInfo.realName}}</span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label>手机号</label> <span>{{accountInfo.mobile}}</span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label>账号密钥</label> <span class="personal-info-form-item-action" v-on:click="showSecretKeyPage"><span>点击查看账号密钥</span><i aria-hidden="true" class="fa fa-angle-right"></i></span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label></label> <span class="personal-info-form-item-action" v-on:click="showGoogleSecretKeyPage"><span>谷歌验证器</span><i aria-hidden="true" class="fa fa-angle-right"></i></span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label></label> <span class="personal-info-form-item-action" v-on:click="$router.push('/bank-card')"><span>我的银行卡</span><i aria-hidden="true" class="fa fa-angle-right"></i></span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label></label> <span class="personal-info-form-item-action" v-on:click="$router.push('/virtual-wallet')"><span>我的电子钱包</span><i aria-hidden="true" class="fa fa-angle-right"></i></span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label>登录密码</label> <span class="personal-info-form-item-action" v-on:click="showModifyLoginPwdPage"><span>点击修改登录密码</span><i aria-hidden="true" class="fa fa-angle-right"></i></span>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-9 offset-sm-3 personal-info-form-item">
						<label>资金密码</label> <span class="personal-info-form-item-action" v-on:click="showModifyMoneyPwdPage"><span>点击修改资金密码</span><i aria-hidden="true" class="fa fa-angle-right"></i></span>
					</div>
				</div>
			</div>

			<form class="modify-login-pwd-form" v-show="modifyLoginPwdFlag">
				<div class="form-group">
					<label>旧的登录密码</label> <input type="text" class="form-control" placeholder="请输入旧的登录密码" v-model="oldLoginPwd">
				</div>
				<div class="form-group">
					<label>新的登录密码</label> <input type="text" class="form-control" placeholder="请输入新的登录密码" v-model="newLoginPwd">
				</div>
				<div class="form-group">
					<label>确认登录密码</label> <input type="text" class="form-control" placeholder="请确认登录密码" v-model="confirmLoginPwd">
				</div>
				<button type="button" class="btn btn-danger btn-lg btn-block" v-on:click="modifyLoginPwd">确认修改</button>
				<button type="button" class="btn btn-light btn-lg btn-block" v-on:click="hideModifyLoginPwdPage">返回</button>
			</form>

			<form class="modify-money-pwd-form" v-show="modifyMoneyPwdFlag">
				<div class="form-group">
					<label>旧的资金密码</label> <input type="text" class="form-control" placeholder="请输入旧的资金密码" v-model="oldMoneyPwd">
				</div>
				<div class="form-group">
					<label>新的资金密码</label> <input type="text" class="form-control" placeholder="请输入新的资金密码" v-model="newMoneyPwd">
				</div>
				<div class="form-group">
					<label>确认资金密码</label> <input type="text" class="form-control" placeholder="请确认资金密码" v-model="confirmMoneyPwd">
				</div>
				<button type="button" class="btn btn-danger btn-lg btn-block" v-on:click="modifyMoneyPwd">确认修改</button>
				<button type="button" class="btn btn-light btn-lg btn-block" v-on:click="hideModifyMoneyPwdPage">返回</button>
			</form>

			<form class="secret-key-form" style="padding-top: 5rem;" v-show="secretKeyFlag">
				<div class="code-content" style="text-align: center;"></div>
				<div class="secret-key-tip">保存截图然后打开监控app进行扫码或手动复制进行配置</div>
				<div style="font-size: 10px; padding-bottom: 1rem;">{{homePageUrl}}/{{secretKey}}</div>
				<button type="button" class="btn btn-danger btn-lg btn-block" v-on:click="updateSecretKey">更换密钥</button>
				<button type="button" class="btn btn-light btn-lg btn-block" v-on:click="hideSecretKeyPage">返回</button>
			</form>

			<form class="secret-key-form" style="padding-top: 3rem;" v-show="bindGoogleAuthFlag">
				<div>请打开谷歌验证器APP，选择"扫描二维码"，或手动输入密钥</div>
				<div style="padding-left: 2rem;">
					<label style="padding-right: 1rem; margin-bottom: 0;">会员账号:</label><span>{{userNameWithGoogleAuth}}</span>
				</div>
				<div style="padding-left: 2rem; padding-right: 2rem;">
					<label style="padding-right: 1rem; margin-bottom: 0;">谷歌密钥:</label><span>{{googleSecretKey}}</span>
					<button type="button" class="btn btn-primary btn-sm" style="float: right;" v-on:click="generateGoogleSecretKey">更换密钥</button>
				</div>
				<div style="text-align: center; padding-top: 0.5rem; padding-bottom: 0.5rem;">
					<div class="code-content"></div>
				</div>
				<div>请输入6位数字谷歌验证码完成绑定操作</div>
				<div style="margin-bottom: 20px;">
					<input type="text" class="form-control" v-model="googleVerCode" placeholder="请输入谷歌验证码">
				</div>
				<button type="button" class="btn btn-danger btn-lg btn-block" v-on:click="bindGoogleAuth">确定</button>
				<button type="button" class="btn btn-light btn-lg btn-block" v-on:click="hideSecretKeyPage">返回</button>
			</form>
		</div>
	</div>
</template>
<script>
import personalInfo from '@/components/common/js/personal-info.js';
export default personalInfo;
</script>
<style scoped>
.personal-info-form {
	padding-left: 2rem;
	padding-top: 5rem;
}

.personal-info-form .row {
	padding-bottom: 1rem;
}

.personal-info-form-item {
	border-bottom: .01563rem solid #f2f2f2;
	height: 2.5rem;
}

.personal-info-form-item label {
	text-align: end;
	width: 6rem;
	margin-right: 1rem;
}

.personal-info-form-item span {
	color: #9e9e9e;
}

.personal-info-form-item-action {
	width: 12rem;
	display: inline-block;
}

.personal-info-form-item-action span {
	padding-right: 3rem;
	width: 11rem;
	display: inline-block;
}

.personal-info-form-item-action i {
	font-size: 1.6rem;
}

.modify-login-pwd-form, .modify-money-pwd-form, .secret-key-form {
	padding-top: 3rem;
	padding-left: 1.5rem;
	padding-right: 1.5rem;
}

.secret-key-tip {
	padding-top: 0.5rem;
	padding-bottom: 0.5rem;
}
</style>


