    ...
    <div id="auditOrder" v-cloak>
		...
		<div class="page-body">
			...
			<div v-show="showWaitConfirmOrderFlag">
				<div class="form-details" v-for="(order, index) in waitConfirmOrders" :key="index">
					...
					<form>
						<div class="form-group">
							<label>申诉类型:</label> <select class="form-control" v-model="appealType">
								...
								<option v-for="(dictItem, index) in appealTypeDictItems" :key="index" :value="dictItem.dictItemCode" v-if="dictItem.dictItemCode == '1' || dictItem.dictItemCode == '2' || dictItem.dictItemCode == '4'">{{dictItem.dictItemName}}</option>
							</select>
						</div>
...
<script>
import auditOrder from '@/components/common/js/audit-order.js';
